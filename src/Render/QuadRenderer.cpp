...
QuadRenderer::QuadRenderer(/* args */)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(Vertex2D), vertecies, GL_STATIC_DRAW);
...
void QuadRenderer::render(Vector2D* vec) const
{
    this->shader->bind();
    ui32 offsetLocation = this->shader->getUniformLocation("in_offset");
    glUniform2f(offsetLocation, vec->x, vec->y);
